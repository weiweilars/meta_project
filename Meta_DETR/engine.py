...
@torch.no_grad()
def sample_support_categories(
    params, targets, support_images, support_class_ids, support_targets
):
    """
    ...
    positive_labels = torch.cat(
        [target["labels"] for target in targets], dim=0
    ).unique()
    ...
    positive_label_indexes = [
        i
        for i in list(range(len(support_images)))
        if support_class_ids[i] in positive_labels_list
    ]
    negative_label_indexes = [
        i
        for i in list(range(len(support_images)))
        if support_class_ids[i] in negative_labels_list
    ]

    meta_support_images, meta_support_class_ids, meta_support_targets = (
        list(),
        list(),
        list(),
    )
    for _ in range(params.episode_num):
        NUM_POS = random.randint(
            max(0, params.episode_size - num_negative_labels),
            min(num_positive_labels, params.episode_size),
        )
        NUM_NEG = params.episode_size - NUM_POS
...
def train_one_epoch(
    ...
    for (
        ...
        support_class_ids,
        support_targets,
    ) in metric_logger.log_every(dataloader, print_freq, header):
        ...
        (
            ...
            args, targets, support_images, support_class_ids, support_targets
        )
        ...
        support_targets = [
            {k: v.to(device) for k, v in t.items()} for t in support_targets
        ]
        ...
        for i in range(num_episode):
            if (args.episode_size * (i + 1)) <= num_classes:
                support_images_ = utils.nested_tensor_from_tensor_list(
                    support_images[
                        (args.episode_size * i) : (args.episode_size * (i + 1))
                    ]
                ).to(device)
                support_targets_ = [
                ...
                support_targets_ = [
                    {k: v.to(device) for k, v in t.items()}
                    for t in support_targets[-args.episode_size :]
                ]
                support_class_ids_ = support_class_ids[-args.episode_size :]
            if isinstance(model, torch.nn.parallel.DistributedDataParallel):
                category_code = model.module.compute_category_codes(
