...
class BackboneBase(nn.Module):
    def __init__(
        ...
        for name, parameter in backbone.named_parameters():
            parameter.requires_grad_(False)  # First freeze all
        if train_backbone:
...
class Backbone(BackboneBase):
    ...
    def __init__(
        self, name: str, train_backbone: bool, return_interm_layers: bool, args
    ):
        self.args = args
...
class Joiner(nn.Sequential):
    ...
    def forward_supp_branch(
        self, tensor_list: NestedTensor, return_interm_layers=False
    ):
        xs = self[0].support_encoding_net(
            tensor_list, return_interm_layers=return_interm_layers
        )
        out: List[NestedTensor] = []
        pos = []
        for name, x in sorted(xs.items()):
...
def build_backbone(params):
    position_embedding = build_position_encoding(params)
    train_backbone = params.lr_backbone > 0
    return_interm_layers = params.num_feature_levels > 1
    backbone = Backbone(params.backbone, train_backbone, return_interm_layers, params)
